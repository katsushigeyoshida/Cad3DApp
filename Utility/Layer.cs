    ...
    public class Layer
    {
        ...
        public List<CheckBoxListItem> getLayerChkList(byte[] layerBits)
        {
            ...
            foreach (KeyValuePair<int, string> item in mLayerList) {
                CheckBoxListItem chkItem = new CheckBoxListItem(ylib.getBitOn(layerBits, item.Key), item.Value);
                chkList.Add(chkItem);
            }
        ...
        public byte[] setLayerChkList(byte[] layerBits, List<CheckBoxListItem> chkList, bool replace = true)
        {
            foreach (CheckBoxListItem chkItem in chkList) {
                ...
                if (n < 0)
                    n = add(chkItem.Text);
                if (chkItem.Checked)
                    ylib.bitOn(layerBits, n);
                else if (replace)
                    ylib.bitOff(layerBits, n);
            }
            return layerBits;
        }
        ...
        public byte[] replaceOn(byte[] layerbit, List<int[]> replaceData)
        {
            ...
            for (int i = 0; i < bitNoList.Count; i++) {
                ...
                if (0 <= n)
                    ylib.bitOn(bytes, replaceData[n][1]);
            }
            return bytes;
        }
        ...
        /// <summary>
        /// bit and で表示レイヤの有無を確認
        /// </summary>
        /// <param name="bytes">レイヤBit</param>
        /// <returns>表示レイヤあり</returns>
        ...
        {
            ylib.bitOnAll(mDispLayerBit, off);
        }
