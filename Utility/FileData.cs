...
namespace Cad3DApp
{
    public class FileData
    {
        ...
        public FileData(Window window, GlobalData global)
        {
            mMainWindow = window;
            this.mGlobal = global;
        }
        ...
        public void setBaseDataFolder(string baseDataFolder = "", bool init = true)
        {
            try {
                if (baseDataFolder != "")
                    mGlobal.mBaseDataFolder = baseDataFolder;
                if (!Directory.Exists(mGlobal.mBaseDataFolder))
                    Directory.CreateDirectory(mGlobal.mBaseDataFolder);
                if (init) {
                    string genreFolder = Path.Combine(mGlobal.mBaseDataFolder, mGenreName);
        ...
        {
            List<string> genreList = ylib.getDirectories(mGlobal.mBaseDataFolder);
            if (genreList != null) {
                genreList.Sort();
        ...
        {
            return Path.Combine(mGlobal.mBaseDataFolder, mGenreName);
        }
        ...
        public string getCurCategoryPath()
        {
            ...
            return Path.Combine(mGlobal.mBaseDataFolder, curCategoryPath);
        }
        ...
        {
            return Path.Combine(mGlobal.mBaseDataFolder, genre);
        }
        ...
        {
            return Path.Combine(mGlobal.mBaseDataFolder, mGenreName + "\\" + category);
        }
        ...
        {
            return Path.Combine(mGlobal.mBaseDataFolder, genreName + "\\" + categoryName);
        }
        ...
        public string getItemFilePath(string dataName, string categoryName, string genreName)
        {
            ...
            return Path.Combine(mGlobal.mBaseDataFolder, curDataFilePath);
        }
        ...
        public bool copyItem(string itemName, bool move = false)
        {
            ...
            dlg.Owner = mMainWindow;
            dlg.WindowStartupLocation = WindowStartupLocation.CenterOwner;
            dlg.mRootFolder = mGlobal.mBaseDataFolder;
            if (dlg.ShowDialog() == true) {
        ...
        /// <returns></returns>
        public int dataBackUp(string srcFolder, string destFolder,bool messageOn = true)
        {
            int count = 0;
            if (srcFolder == null || destFolder.Length == 0) {
                ylib.messageBox(mMainWindow, "図面データのバックアップのフォルダが設定されていません。");
                return count;
            }
            if (!Directory.Exists(destFolder))
                Directory.CreateDirectory(destFolder);
            if (Path.GetFullPath(srcFolder) != Path.GetFullPath(destFolder)) {
                DirectoryDiff directoryDiff = new DirectoryDiff(srcFolder, destFolder);
        ...
        /// バックアップデータ管理
        /// </summary>
        public void dataRestor(string srcFolder, string destFolder)
        {
            if (mGlobal.mBaseDataFolder == null || mGlobal.mBackupFolder.Length == 0 || !Directory.Exists(mGlobal.mBackupFolder)) {
            ...
            dlg.mSrcFolder = srcFolder;
            dlg.mDestFolder = destFolder;
